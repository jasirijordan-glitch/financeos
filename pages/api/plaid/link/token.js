  ...
  try{
    const r=await fetch(base+'/link/token/create',{
      ...
      body:JSON.stringify({
        client_id:clientId,
        secret,
        user:{client_user_id:'financeos-user'},
    ...
    if(d.link_token) return res.status(200).json({linkToken:d.link_token});
    return res.status(400).json({error:d.error_message||'Plaid API error'});
