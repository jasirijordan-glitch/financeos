...
// ──Ã¢ÂÂ Safe API client (inline — no import chain that can fail) ────────
const api = (() => {
...
// ──Ã¢ÂÂ Plan Capability System ──────────────────────────────────────Ã¢ÂÂ
...
// ──Ã¢ÂÂ Financial Data Constants ────────────────────────────────────Ã¢ÂÂ
const BASE_PNL = {
...
// ──Ã¢ÂÂ FeatureGate — generic feature-flag wrapper ──────────────────
/**
...
// ──Ã¢ÂÂ LOCKED_COPY — tab-specific locked state messaging ────────────
const LOCKED_COPY = {
  scenario: {
...
// ──Ã¢ÂÂ Onboarding Checklist ────────────────────────────────────────Ã¢ÂÂ
/**
              ...
              <div key={sc.id} style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:10,padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}>
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:10,color:T.textDim,fontFamily:T.mono}}>
                    v{latest?.version || 1} Â· {latest?.label || "—"}
  ...
  return (
    <div style={{
      ...
      <div style={{height:56,flexShrink:0,display:"flex",alignItems:"center",gap:12,padding:"0 20px",borderBottom:`1px solid ${T.border}`,background:`linear-gradient(90deg,${T.card},${T.surface})`,cursor:"pointer"}} onClick={()=>setOpen(o=>!o)}>
        <div style={{width:30,height:30,borderRadius:"50%",background:`linear-gradient(135deg,${T.cyan},${T.violet})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,boxShadow:`0 0 14px ${T.cyan}60`,flexShrink:0}}>✦</div>
          ...
          <div style={{display:"flex",flexDirection:"column",background:T.card}}>
            ...
            {rightTab==="chat" && (
              <>
                ...
                {hasFeature(plan,FEATURES.FULL_AI)&&<div style={{padding:"0 12px 8px",fontSize:9,color:T.textDim,fontFamily:T.sans}}>Powered by Claude Â· {hasFeature(plan,FEATURES.FULL_AI)?"Context-aware of all on-screen data":"Basic mode — upgrade for full financial context"}</div>}
              </>
            )}
...
// ──Ã¢ÂÂ Monthly Revenue vs. Net Income Chart ────────────────────────Ã¢ÂÂ
function RevNetChart({ pnl }) {
  ...
  // ── Right Y-axis: Net Income  (−5K → +25K) ──────────────────────Ã¢ÂÂ
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
        ...
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px 18px",marginBottom:20}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14}}>📊 Monthly Revenue vs. Net Income</div>
        ...
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,overflow:"hidden",marginBottom:20}}>
          <div style={{padding:"14px 16px",borderBottom:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14}}>📋 Full P&L Statement — FY {new Date().getFullYear()}</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:20}}>
        ...
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px 18px",marginBottom:20}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
            <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14}}>🎛️ Custom Scenario Builder</div>
            {!canUseNumInput && (
        ...
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px 18px"}}>
          <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14,marginBottom:14}}>📉 Income Waterfall — {useC?"Custom":SCENARIOS_DEF[active].label} Scenario</div>
          {wf.map(d=>(
...
// ──Ã¢ÂÂ Cash Flow Forecast ──────────────────────────────────────────Ã¢ÂÂ
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:18}}>
        ...
        {view==="bars"&&(
          <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px 18px",marginBottom:18}}>
            <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14,marginBottom:14}}>💧 Weekly Cash Flow — 13-Week Forecast</div>
        ...
        {view==="table"&&(
          <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,overflow:"hidden",marginBottom:18}}>
            <div style={{padding:"14px 16px",borderBottom:`1px solid ${T.border}`,color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14}}>📋 13-Week Cash Flow Detail</div>
            <div style={{overflowX:"auto"}}>
        ...
        {view==="runway"&&(
          <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px 18px",marginBottom:18}}>
            <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14,marginBottom:14}}>📈 13-Week Balance Runway</div>
        ...
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px 18px"}}>
          <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14,marginBottom:14}}>💸 13-Week Outflow Composition</div>
          {CATS.map(cat=>{
    ...
    <div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10,marginBottom:18}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:18}}>
          <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px 18px"}}>
            <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14,marginBottom:14}}>📊 AR Distribution</div>
          ...
          <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px 18px"}}>
            <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14,marginBottom:14}}>📏 Collections Health</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        ...
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,overflow:"hidden"}}>
          <div style={{padding:"12px 16px",borderBottom:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14}}>📬 AR Aging — {sorted.length} Clients</div>
    ...
    <div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:18}}>
        ...
        {view==="matrix"&&(
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px 18px"}}>
              <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14,marginBottom:14}}>📊 Revenue vs. Margin Scatter (bubble = NPS)</div>
...
// ──Ã¢ÂÂ Integrations Field Input ────────────────────────────────────Ã¢ÂÂ
...
function CFOSimulation({ plan="professional", aiContext={} }) {
  ...
  // ── Inline AI assistant state ──────────────────────────────────Ã¢ÂÂ
    ...
    <div style={{maxWidth:1100,margin:"0 auto"}}>
      ...
      <div style={{textAlign:"center",marginBottom:44}}>
        ...
        <div style={{color:T.textDim,fontFamily:T.sans,fontSize:14,marginBottom:24,maxWidth:440,margin:"0 auto 24px"}}>
          Trusted by 500+ growing businesses. Start free — upgrade when you're ready to plan, forecast, and scale with confidence.
        </div>
        {/* Billing toggle */}
            ...
            <div key={plan.id} className={isCenter?"pricing-pro-glow":""}
              ...
              {isCenter&&(
                <div style={{background:`linear-gradient(90deg,${T.cyan},${T.violet})`,padding:"6px 0",textAlign:"center"}}>
                  <span style={{color:T.bg,fontSize:10,fontFamily:T.mono,fontWeight:800,letterSpacing:1.5,textTransform:"uppercase"}}>⭐ Most Popular Â· Best for Small Business</span>
                </div>
              )}
              {isCurrent&&(
    ...
    <div>
        ...
        {[
          ...
          return (
            <div key={k.l} style={{background:T.card,border:`1px solid ${good?c+"40":T.border}`,borderRadius:12,padding:"14px 16px",boxShadow:good?"none":`0 0 16px ${T.rose}10`}}>
              ...
              {!k.isRatio&&<div style={{fontSize:10,color:c,fontFamily:T.sans,marginTop:4}}>{good?"▲ Favorable":"▼ Unfavorable"} Â· {pct(Math.abs(k.pct||0))} vs budget</div>}
            </div>
          );
        })}
      </div>
      ...
      {view==="summary"&&(
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,overflow:"hidden"}}>
            <div style={{padding:"12px 16px",borderBottom:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14}}>📐 Budget vs. Actuals — FY {new Date().getFullYear()}</div>
          ...
          <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px 18px"}}>
            <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14,marginBottom:14}}>📅 Monthly Variance Heatmap</div>
      ...
      {view==="rolling"&&(
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px 18px"}}>
            <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14,marginBottom:4}}>🔄 Rolling Forecast — Updated Monthly</div>
  ...
  return (
    <div>
      ...
      {view==="cost"&&(
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px 18px"}}>
            <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14,marginBottom:2}}>💰 Payroll Cost by Department</div>
  ...
  return (
    <div>
      ...
      {view==="overview"&&(
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px 18px"}}>
            <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14,marginBottom:14}}>📈 MRR Trend — FY {new Date().getFullYear()}</div>
      ...
      {view==="waterfall"&&(
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px 18px"}}>
          <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14,marginBottom:14}}>🌊 MRR Waterfall — Monthly Movement</div>
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:10}}>
              <thead><tr style={{background:T.surface}}>
      ...
      {view==="customers"&&(
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px 18px"}}>
          {/* Header + legend row */}
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
            <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14}}>👥 Customer Growth — Acquired vs. Churned</div>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
      ...
      {view==="unit"&&(
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px 18px"}}>
            <div style={{color:T.text,fontFamily:T.display,fontWeight:700,fontSize:14,marginBottom:14}}>📊 Net Revenue Retention Trend</div>
...
// ──Ã¢ÂÂ C-Suite Strategic Highlights & Watch Items ──────────────────Ã¢ÂÂ
const CSUITE_ROLES = {
  CEO: {
      ...
      <div style={{background:`linear-gradient(180deg,${T.surface} 0%,${T.bg}ee 100%)`,borderBottom:`1px solid ${T.border}`,padding:"0 24px",display:"flex",alignItems:"stretch",position:"sticky",top:0,zIndex:100,flexDirection:"column",backdropFilter:"blur(12px)"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:9,flexShrink:0,paddingRight:16,borderRight:`1px solid ${T.border}`,height:60,alignSelf:"stretch",alignContent:"center",flexWrap:"wrap"}}>
            <div style={{width:38,height:38,borderRadius:10,background:`linear-gradient(135deg,${T.cyan},${T.violet})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,boxShadow:`0 0 20px ${T.cyan}50`,flexShrink:0}}>⬡</div>
            <div>
